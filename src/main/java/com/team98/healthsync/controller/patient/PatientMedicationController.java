...
public class PatientMedicationController {
    ...
    @ResponseBody
    private Iterable<Medication> getPatMedications(){
        return medicationRepository.findAllByAppointment_Patient_Id(patientHelper.GetPatientId());
    }


    @GetMapping("/patient/medications/{id}")
    public ModelAndView viewPatMedication(@PathVariable("id") @NotNull @Min(value = 0) long id){
        ...
        mv.setViewName("thymeleaf/patient/medication/view_medication");
        return mv;
    }
